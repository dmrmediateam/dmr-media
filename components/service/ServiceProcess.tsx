    ...
    <section id={id} className="py-24 bg-white">
      <div className="container-max">
        <div className="max-w-3xl mb-16">
          ...
          {description && (
            <p className="mt-6 text-sm sm:text-base text-[var(--color-ink-300)] leading-relaxed">
              {description}
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {steps.map((step, index) => (
            <div key={step.title} className="rounded-[24px] border border-[var(--color-ink-200)] bg-white/80 backdrop-blur-sm p-8">
              <div className="text-xs uppercase tracking-[0.3em] text-[var(--color-ink-300)] mb-3">
